...
# 検索対処の型名(正規表現)
preprocessor_word = ["#define", "#pragma", "#ifdef", "#endif", "#if", "#else", "#include", "#error", "#warning"]
#検索結果格納
result = []
#csvのヘッダー行
...
# 検索ワードを作成
search_word = []
for word in preprocessor_word:
	# 行先頭に宣言されていないプリプロセッサを抽出
	search_word.append(r"^(?!{0}).*(?={0}).+$".format(word))
