...
# 検索対処の型名(正規表現)
search_type = ["U2", "U4", "S4", "VD", "void", "VS1", "VS2", "VS4", "VU1", "VU2", "VU4", "BL", "F4"]
# 検索除外対象(正規表現)
exclusions = ["^#define", "^#ifdef"]
#検索結果格納
result = []
#csvのヘッダー行
result.append(("\"型\"", "\"ファイル名\"", "\"行数\"", "\"宣言部\""))
...
for file in file_list:
	...
	for i, line in enumerate(Common.utilities.read_line(file)):
		...
		# 型検索ループ
		for type_ in search_type:
			regex = type_
			search_result = re.search(regex, line)
			if search_result != None:
				# "line[:-1]"は末尾改行を削除している(末尾は必ず改行があること前提)
				tmp = (type_, relative_path, i + 1, line[:-1])
				result.append(tmp)
				break

# 除外対象が含めれている物は結果リストから削除
for result_line in result[:]:
	for exclusion in exclusions:
		regex = exclusion
		search_result = re.search(regex, result_line[3])
		if search_result != None:
			result.remove(result_line)
